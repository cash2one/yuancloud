...
execfile(join(dirname(__file__), 'yuancloud', 'release.py'))  # Load release variables
lib_name = 'yuancloud'
...
def py2exe_datafiles():
    ...
    for root, dirnames, filenames in os.walk('yuancloud'):
...
def py2exe_options():
    if os.name == 'nt':
        ...
        return {
            'console': [
                {'script': 'yuancloud.py'},
                {'script': 'yuancloud-gevent'},
                {'script': 'yuancloud-server', 'icon_resources': [
                    (1, join('setup', 'win32', 'static', 'pixmaps', 'yuancloud-icon.ico'))
                ]},
            ],
            'options': {
                'py2exe': {
...
setup(
    ...
    license=license,
    scripts=['yuancloud-server', 'yuancloud.py'],
    packages=find_packages(),
    package_dir={'%s' % lib_name: 'yuancloud'},
