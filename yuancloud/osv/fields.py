...
from yuancloud.tools import float_repr, float_round, frozendict, html_sanitize
import json
...
class _column(object):
    ...
    def to_field(self):
        ...
        from yuancloud.fields import Field
    ...
    def as_display_name(self, cr, uid, obj, value, context=None):
        """Converts a field value to a suitable string representation for a record,
           ...
           :param value: a proper value as returned by :py:meth:`~yuancloud.orm.osv.BaseModel.read`
                         for this column
        """
...
class binary(_column):
    ...
    # the legacy YuanCloud convention is to transfer and store binaries
        ...
        if not all((tbl, col1, col2)):
            ...
            if not tbl:
                assert tables[0] != tables[1], 'Implicit/Canonical naming of m2m relationship table '\
                                               ...
                                               'the same'
                tbl = '%s_%s_rel' % tables
                yuancloud.models.check_pg_name(tbl)
            if not col1:
            ...
            for field_name in field_names:
                ...
                if field_type in ('one2many','many2many'):
                    ...
                    if value:
                        ...
                        value = relation_obj.exists(cr, yuancloud.SUPERUSER_ID, value)
                if type(value) in (int,long) and field_type == 'many2one':
                    relation_obj = obj.pool[obj._columns[field_name].relation]
                    # check for deleted record as superuser
                    if not relation_obj.exists(cr, yuancloud.SUPERUSER_ID, [value]):
                        value = False
