# -*- coding: utf-8 -*-
"""
The module :mod:`yuancloud.tests.common` provides unittest test cases and a few
...
# The yuancloud library is supposed already configured.
ADDONS_PATH = yuancloud.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = yuancloud.tools.config['xmlrpc_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = yuancloud.SUPERUSER_ID


def get_db_name():
    db = yuancloud.tools.config['db_name']
...
class BaseCase(unittest.TestCase):
    """
    Subclass of TestCase for common YuanCloud-specific code.
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~yuancloud.models.BaseModel`
        """
        assert "." in xid, "this method requires a fully qualified parameter, in the following form: 'module.identifier'"
        module, xid = xid.split('.')
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = yuancloud.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        yuancloud.http.root.session_store.save(self.session)
    ...
    def authenticate(self, user, password):
        ...
        # yuancloudSession.authenticate accesses the current request, which we
        ...
        yuancloud.http.root.session_store.save(session)
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('yuancloud.service.http.request.'):
                while thread.isAlive():
                    ...
                    if t0 != t1:
                        ...
                        yuancloud.tools.misc.dumpstacks()
                        t0 = t1
