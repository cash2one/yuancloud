...
def check_object_name(name):
    """ Check if the given name is a valid yuancloud object name.
...
class MetaModel(api.Meta):
    ...
    def __init__(self, name, bases, attrs):
        ...
        if not hasattr(self, '_module'):
            # The (YuanCloud) module name can be in the ``yuancloud.addons`` namespace
            # or not.  For instance, module ``sale`` can be imported as
            # ``yuancloud.addons.sale`` (the right way) or ``sale`` (for backward
            # compatibility).
            module_parts = self.__module__.split('.')
            if len(module_parts) > 2 and module_parts[:2] == ['yuancloud', 'addons']:
...
class BaseModel(object):
    """ Base class for YuanCloud models.

    YuanCloud models are created by inheriting from this class' subclasses:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _add_fake_fields(self, cr, uid, fields, context=None):
        from yuancloud.fields import Char, Integer
    ...
    def _read_group_fill_results(self, cr, uid, domain, groupby, remaining_groupbys,
        ...
        all_groups,folded = self._group_by_full[groupby](self, cr, uid, present_group_ids, domain,
                                                  ...
                                                  access_rights_uid=yuancloud.SUPERUSER_ID,
                                                  context=context)
                            ...
                            if f.required:
                                try:
                                    ...
                                    msg = "WARNING: unable to set column %s of table %s not null !\n"\
                                        "Try to re-run: yuancloud-server --update=module\n"\
    ...
    def create_workflow(self, cr, uid, ids, context=None):
        ...
        from yuancloud import workflow
        for res_id in ids:
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~yuancloud.fields.Integer`,
          :class:`~yuancloud.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~yuancloud.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~yuancloud.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~yuancloud.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~yuancloud.fields.Date` and
              :class:`~yuancloud.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`yuancloud.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`yuancloud.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _yuancloud/models/relationals/format:

          :class:`~yuancloud.fields.One2many` and
          :class:`~yuancloud.fields.Many2many` use a special "commands" format to
          ...
          ``(3, id, _)``
              ...
              :class:`~yuancloud.fields.One2many`. Can not be used in
              :meth:`~.create`.
          ``(4, id, _)``
              adds an existing record of id ``id`` to the set. Can not be
              used on :class:`~yuancloud.fields.One2many`.
          ``(5, _, _)``
          ...
          ``(6, _, ids)``
              ...
              :class:`~yuancloud.fields.One2many`.
    ...
    def _where_calc(self, cr, user, domain, active_test=True, context=None):
        """Computes the WHERE clause needed to implement an YuanCloud domain.
    ...
    def _generate_order_by(self, order_spec, query):
        _order_str=self._order
        # if 'write_date' in self._columns.keys():
        #     if order_spec:
        #         order_spec ="write_date desc, %s" % order_spec
        #     if self._order:
        #         _order_str='write_date desc,%s' % self._order
        ...
        """
        order_by_clause = ''
        order_spec = order_spec or _order_str
        if order_spec:
    ...
    def with_env(self, env):
        ...
        :type env: :class:`~yuancloud.api.Environment`
        """
...
class Model(BaseModel):
    """Main super-class for regular database-persisted YuanCloud models.

    YuanCloud models are created by inheriting from this class::
