yuancloud.define('website_links.website_links', function (require) {
'use strict';
...
if(!$('.o_website_links_create_tracked_url').length) {
    ...
    base.ready().done(function() {
        ...
        var officialaccount_select= new SelectBox('wx.officialaccount');
        officialaccount_select.start($("#officialaccount-select"),_t('e.g.　山水物源'));

        var sales_person_select= new SelectBox('res.users');
        sales_person_select.start($("#sales_person-select"),_t('e.g.　admin'));

        var sale_team_select= new SelectBox('crm.team');
        sale_team_select.start($("#sale_team-select"),_t('e.g.　网站销售'));
        ...
        $('#o_website_links_link_tracker_form').unbind();
        ...
        $("#o_website_links_link_tracker_form").submit(function(event) {
            ...
            var officialaccount_id=$('#officialaccount-select').attr('value');
            var sales_person_id=$('#sales_person-select').attr('value');
            var sale_team_id=$('#sale_team-select').attr('value');
            ...
            if(officialaccount_id !== '') { params.officialaccount = parseInt(officialaccount_id); }
            if(sales_person_id !== '') { params.user_id = parseInt(sales_person_id); }
            if(sale_team_id !== '') { params.team_id = parseInt(sale_team_id); }
            if(share_desc!=""){params.share_desc=share_desc;}
            ...
            ajax.jsonRpc("/website_links/new", 'call', params)
                .then(function (result) {
                    ...
                    if('error' in result) {
                        ...
                        $('#officialaccount-select').select2('val', '');
                        $('#sales_person-select').select2('val', '');
                        $('#sale_team-select').select2('val', '');
