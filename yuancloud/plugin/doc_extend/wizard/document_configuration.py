...
class document_configuration(osv.osv_memory):
    ...
    def execute(self, cr, uid, ids, context=None):
        ...
        if self.pool.get('sale.order'):
            # Sale order
            dir_data_id = data_pool._get_id(cr, uid, 'doc_extend', 'dir_sale_order_all')
            if dir_data_id:
            ...
            # Qutation
            dir_data_id = data_pool._get_id(cr, uid, 'doc_extend', 'dir_sale_order_quote')
            if dir_data_id:
        ...
        if self.pool.get('product.product'):
            # Product
            dir_data_id = data_pool._get_id(cr, uid, 'doc_extend', 'dir_product')
            if dir_data_id:
        ...
        if self.pool.get('account.analytic.account'):
            # Project
            dir_data_id = data_pool._get_id(cr, uid, 'doc_extend', 'dir_project')
            if dir_data_id:
