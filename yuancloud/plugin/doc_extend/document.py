...
from yuancloud.osv.orm import except_orm
import yuancloud.report.interface
from yuancloud.tools.misc import ustr
...
class document_directory_content(osv.osv):
    ...
    def process_read(self, cr, uid, node, context=None):
        ...
        srv = yuancloud.report.interface.report_int._reports['report.'+report.report_name]
...
class node_context(object):
    ...
    def __init__(self, cr, uid, context=None):
        ...
        if context is None:
            context = {}
        context['uid'] = uid
        self._dirobj = yuancloud.registry(cr.dbname).get('document.directory')
...
class nodefd_content(StringIO, node_descriptor):
    ...
    def close(self):
        ...
        uid = par.context.uid
        cr = yuancloud.registry(par.context.dbname).cursor()
        try:
...
class nodefd_db(StringIO, node_descriptor):
    ...
    def close(self):
        ...
        par = self._get_parent()
        # uid = par.context.uid
        registry = yuancloud.modules.registry.RegistryManager.get(par.context.dbname)
