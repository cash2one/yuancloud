from yuancloud.tests.common import TransactionCase
from yuancloud import netsvc, workflow
...
class TestCheckJournalEntry(TransactionCase):
    ...
    def setUp(self):
        ...
        self.expense_obj = self.registry('hr.expense8.expense')
        self.exp_line_obj = self.registry('hr.expense8.line')
    ...
    def test_journal_entry(self):
        cr, uid = self.cr, self.uid
        #Submit to Manager
        workflow.trg_validate(uid, 'hr.expense8.expense', self.expense_id, 'confirm', cr)
        #Approve
        workflow.trg_validate(uid, 'hr.expense8.expense', self.expense_id, 'validate', cr)
        #Create Expense Entries
        workflow.trg_validate(uid, 'hr.expense8.expense', self.expense_id, 'done', cr)
