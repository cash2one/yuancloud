...
class hr_expense_expense(osv.osv):
    ...
    def _get_expense_from_line(self, cr, uid, ids, context=None):
        return [line.expense_id.id for line in self.pool.get('hr.expense8.line').browse(cr, uid, ids, context=context)]
    ...
    _columns = {
        ...
        'line_ids': fields.one2many('hr.expense8.line', 'expense_id', 'Expense Lines', copy=True,
                                    readonly=True, states={'draft':[('readonly',False)]} ),
        'note': fields.text('Note'),
        'amount': fields.function(_amount, string='Total Amount', digits_compute=dp.get_precision('Account'), 
            store={
                'hr.expense8.line': (_get_expense_from_line, ['unit_amount','unit_quantity'], 10)
            }),
    ...
    def onchange_currency_id(self, cr, uid, ids, currency_id=False, company_id=False, context=None):
        ...
        journal_ids = self.pool.get('account.journal').search(cr, uid, [('type','=','purchase'), ('company_id', '=', company_id)], context=context)
        if journal_ids:
...
class hr_expense_line(osv.osv):
    ...
    def _amount(self, cr, uid, ids, field_name, arg, context=None):
        if not ids:
            return {}
        cr.execute("SELECT l.id,COALESCE(SUM(l.unit_amount*l.unit_quantity),0) AS amount FROM hr_expense8_line l WHERE id IN %s GROUP BY l.id ",(tuple(ids),))
    ...
    _columns = {
        ...
        'expense_id': fields.many2one('hr.expense8.expense', 'Expense', ondelete='cascade', select=True),
