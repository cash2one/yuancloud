from yuancloud.addons.saas_base.tools import get_size
import time
import yuancloud
from datetime import datetime
from yuancloud.service import db
from yuancloud import api, models, fields, SUPERUSER_ID, exceptions
...
class SaasServerClient(models.Model):
    ...
    def create_database(self, template_db=None, demo=False, lang='en_US'):
        new_db = self.name
        if template_db:
            yuancloud.service.db._drop_conn(self.env.cr, template_db)
            yuancloud.service.db.exp_duplicate_database(template_db, new_db)
        else:
            password = ''.join(random.SystemRandom().choice(string.ascii_uppercase + string.digits) for _ in range(32))
            yuancloud.service.db.exp_create_database(new_db, demo, lang, user_password='admin')
        self.state = 'open'

    @api.one
    def registry(self, new=False, **kwargs):
        m = yuancloud.modules.registry.RegistryManager
        if new:
    ...
    @api.one
    def _prepare_database(self, client_env, owner_user=None, is_template_db=False, addons=[], access_token=None,
                          tz=None):
    ...
    def _get_data(self, client_env, check_client_id):
        ...
        users_len = len(users)
        data_dir = yuancloud.tools.config['data_dir']
    ...
    def delete_expired_databases(self):
        ...
        res = self.search(
            [('state', 'not in', ['deleted', 'template']), ('expiration_datetime', '<=', now), ('trial', '=', True)])
        _logger.info('delete_expired_databases %s', res)
        res.delete_database()

    @api.one
    def delete_database(self):
        yuancloud.service.db.exp_drop(self.name)
        self.write({'state': 'deleted'})

    @api.one
    def rename_database(self, new_dbname):
        yuancloud.service.db.exp_rename(self.name, new_dbname)
