...
from yuancloud.addons.auth_oauth.controllers import main as oauth
from yuancloud.addons.auth_signup.controllers import main as signup
import werkzeug
import simplejson
import uuid
import random
...
class SaasPortal(http.Controller):
    ...
    def exists_database(self, dbname):
        full_dbname = self.get_full_dbname(dbname)
        return yuancloud.service.db.exp_db_exist(full_dbname)
...
class OAuthLogin(oauth.OAuthLogin):
    ...
    def web_login(self, *args, **kw):
        if kw.get('login', False):
            ...
            if data and data[0]['share'] and 'database' in data[0] and data[0]['database']:
