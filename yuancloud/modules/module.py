...
class AddonsImportHook(object):
    """
    Import hook to load YuanCloud addons from multiple paths.

    YuanCloud implements its own import-hook to load its addons. YuanCloud
    ...
    are living in `yuancloud.addons`. The good way to import such modules is
    thus `import yuancloud.addons.module`.
    """

    def find_module(self, module_name, package_path=None):
        module_parts = module_name.split('.')
        if len(module_parts) == 3 and module_name.startswith('yuancloud.addons.'):
    ...
    def load_module(self, module_name):
        ...
        mod = imp.load_module('yuancloud.addons.' + module_part, f, path, descr)
        sys.modules['yuancloud.addons.' + module_part] = mod
        return mod
...
def get_module_filetree(module, dir='.'):
    ...
    files = yuancloud.tools.osutil.listdir(path, True)
...
def init_module_models(cr, module_name, obj_list):
    ...
    for t in todo:
        t[1](cr, *t[2])
    if obj_list:
        obj_list[0].recompute(cr, yuancloud.SUPERUSER_ID, {})
    cr.commit()

def load_yuancloud_module(module_name):
    """ Load an YuanCloud module, if not already loaded.
    ...
    try:
        ...
        __import__('yuancloud.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['yuancloud.addons.' + module_name], info['post_load'])()
...
def get_test_modules(module):
    ...
    modpath = 'yuancloud.addons.' + module
    try:
...
class TestStream(object):
    def __init__(self, logger_name='yuancloud.tests'):
...
def run_unit_tests(module_name, dbname, position=runs_at_install):
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, yuancloud.sql_db.sql_counter - t0_sql)
