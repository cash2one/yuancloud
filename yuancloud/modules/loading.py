...
import yuancloud
import yuancloud.modules.db
import yuancloud.modules.graph
import yuancloud.modules.migration
...
from yuancloud.modules.module import initialize_sys_path, \
    load_yuancloud_module, init_module_models, adapt_version
from module import runs_post_install

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('yuancloud.tests')
...
def load_module_graph(cr, graph, status=None, perform_checks=True, skip_modules=None, report=None):
    ...
    def load_test(module_name, idref, mode):
        ...
        try:
            ...
            if tools.config.options['test_commit']:
                ...
                yuancloud.modules.registry.RegistryManager.clear_caches(cr.dbname)
    ...
    migrations = yuancloud.modules.migration.MigrationManager(cr, graph)
    ...
    t0 = time.time()
    t0_sql = yuancloud.sql_db.sql_counter
    ...
    for index, package in enumerate(graph):
        ...
        load_yuancloud_module(package.name)

        new_install = package.state == 'to install'
        if new_install:
            py_module = sys.modules['yuancloud.addons.%s' % (module_name,)]
        ...
        if hasattr(package, 'init') or hasattr(package, 'update') or package.state in ('to install', 'to upgrade'):
            ...
            modobj.update_translations(cr, SUPERUSER_ID, [module_id], None, {'overwrite': yuancloud.tools.config["overwrite_existing_translations"]})
            ...
            if has_demo:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(yuancloud.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, yuancloud.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    cr = db.cursor()
    try:
        if not yuancloud.modules.db.is_initialized(cr):
        ...
        # yuancloud.modules.registry.RegistryManager.new().
        registry = yuancloud.registry(cr.dbname)
        ...
        graph = yuancloud.modules.graph.Graph()
        ...
        if processed_modules:
            ...
            for (model, name) in cr.fetchall():
                if model in registry and not registry[model].is_transient() and not isinstance(registry[model], yuancloud.osv.orm.AbstractModel):
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['yuancloud.addons.%s' % (pkg.name,)]
                ...
                yuancloud.api.Environment.reset()
                return yuancloud.modules.registry.RegistryManager.new(cr.dbname, force_demo, status, update_module)
        ...
        t0 = time.time()
        t0_sql = yuancloud.sql_db.sql_counter
        if yuancloud.tools.config['test_enable']:
            cr.execute("SELECT name FROM ir_module_module WHERE state='installed'")
            for module_name in cr.fetchall():
                report.record_result(yuancloud.modules.module.run_unit_tests(module_name[0], cr.dbname, position=runs_post_install))
            _logger.log(25, "All post-tested in %.2fs, %s queries", time.time() - t0, yuancloud.sql_db.sql_counter - t0_sql)
    finally:
        cr.close()
