...
from yuancloud.tools import assertion_report, lazy_property, classproperty, config, topological_sort
from yuancloud.tools.lru import LRU
...
class Registry(Mapping):
    ...
    def __init__(self, db_name):
        ...
        self._db = yuancloud.sql_db.db_connect(db_name)
        ...
        # Inter-process signaling (used only when yuancloud.multi_process is True):
        ...
        cr = self.cursor()
        has_unaccent = yuancloud.modules.db.has_unaccent(cr)
        if yuancloud.tools.config['unaccent'] and not has_unaccent:
            _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
        self.has_unaccent = yuancloud.tools.config['unaccent'] and has_unaccent
        cr.close()
    ...
    def setup_multi_process_signaling(cls, cr):
        if not yuancloud.multi_process:
            return None, None
...
class RegistryManager(object):
        ...
        with cls.lock():
            with yuancloud.api.Environment.manage():
                ...
                try:
                    ...
                    yuancloud.modules.load_modules(registry._db, force_demo, status, update_module)
                except Exception:
    ...
    def check_registry_signaling(cls, db_name):
        ...
        """
        changed = False
        if yuancloud.multi_process and db_name in cls.registries:
