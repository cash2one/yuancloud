...
import yuancloud
from yuancloud.service.server import memory_info
from yuancloud.service import security, model as service_model
from yuancloud.tools.func import lazy_property
from yuancloud.tools import ustr, consteq
...
# *broken*
NO_POSTMORTEM = (yuancloud.osv.orm.except_orm,
                 yuancloud.exceptions.AccessError,
                 yuancloud.exceptions.ValidationError,
                 yuancloud.exceptions.MissingError,
                 yuancloud.exceptions.AccessDenied,
                 yuancloud.exceptions.Warning,
                 yuancloud.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                yuancloud.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method),
                                     replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = yuancloud.service.common.dispatch
        elif service_name == 'db':
            dispatch = yuancloud.service.db.dispatch
        elif service_name == 'object':
            dispatch = yuancloud.service.model.dispatch
        elif service_name == 'report':
            dispatch = yuancloud.service.report.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            logline = '%s.%s time:%.3fs mem: %sk -> %sk (diff: %sk)' % (
                service_name, method, end_time - start_time, start_vms / 1024, end_vms / 1024,
                (end_vms - start_vms) / 1024)
            if rpc_response_flag:
                yuancloud.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                yuancloud.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except yuancloud.exceptions.DeferredException, e:
        _logger.exception(yuancloud.tools.exception_to_unicode(e))
        yuancloud.tools.debugger.post_mortem(yuancloud.tools.config, e.traceback)
        raise
    except Exception, e:
        _logger.exception(yuancloud.tools.exception_to_unicode(e))
        yuancloud.tools.debugger.post_mortem(yuancloud.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all YuanCloud Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in yuancloud.service.wsgi_server.application
        if self.db:
    ...
    def env(self):
        """
        The :class:`~yuancloud.api.Environment` bound to current request.
        Raises a :class:`RuntimeError` if the current requests is not bound
        to a database.
        """
        if not self.db:
            return RuntimeError('request not bound to a database')
        return yuancloud.api.Environment(self.cr, self.uid, self.context)
    ...
    def session(self):
        """
        a :class:`yuancloudSession` holding the HTTP session data for the
    ...
    def cr(self):
        """
        :class:`~yuancloud.sql_db.Cursor` initialized for the current method
        call.
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            yuancloud.tools.debugger.post_mortem(
                yuancloud.tools.config, sys.exc_info())
        raise
    ...
    def session_id(self):
        """
        opaque identifier for the :class:`yuancloudSession` instance of
        the current request
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return yuancloud.modules.registry.RegistryManager.get(self.db) if self.db else None
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        YuanCloud implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <yuancloud.http.WebRequest.csrf_token`, the
          :data:`~yuancloud.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          YuanCloud) as e.g. it is a REST API or a `webhook
    ...
    if 'auth_list' in yuancloud.tools.config.options:
        auth_list = [x.strip() for x in yuancloud.tools.config['auth_list'].split(',')]  # ['/shop', '/blog', '/forum']
        rs = route if isinstance(route, list) else [route]
        if route and [s for s in rs if any(s.startswith(xs) for xs in auth_list)]:
            routing['auth'] = 'user'
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception,
                              (yuancloud.exceptions.Warning, SessionExpiredException, yuancloud.exceptions.except_orm)):
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "YuanCloud Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "YuanCloud Session Expired"
...
def serialize_exception(e):
    ...
    if isinstance(e, yuancloud.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, yuancloud.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, yuancloud.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, yuancloud.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, yuancloud.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, yuancloud.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, yuancloud.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, yuancloud.exceptions.except_orm):
...
def jsonrequest(f):
    """ 
        .. deprecated:: 8.0
            Use the :func:`~yuancloud.http.route` decorator instead.
    """
...
YuanCloud URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.yuancloud.cn/documentation/9.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through YuanCloud via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["yuancloud", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('yuancloud.addons.') and c.__module__.split(".")[2] in modules
...
class yuancloudSession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(yuancloudSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def _fix_lang(self, context):
        """ YuanCloud provides languages which may not make sense and/or may not
    ...
    def assert_valid(self, force=False):
        ...
        Ensures this session is valid (logged into the yuancloud server)
        """
    ...
    def model(self, model):
        ...
        :param model: an YuanCloud model name
        :type model: str
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        uid = self.uid or request.uid or yuancloud.SUPERUSER_ID
...
class Root(object):
    """Root WSGI application for the YuanCloud Web Client.
    """
    ...
    def session_store(self):
        # Setup http sessions
        path = yuancloud.tools.config.session_dir
        _logger.debug('HTTP sessions stored in: %s', path)
        return werkzeug.contrib.sessions.FilesystemSessionStore(path, session_class=yuancloudSession)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        return routing_map([''] + yuancloud.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        for addons_path in yuancloud.modules.module.ad_paths:
            for module in sorted(os.listdir(str(addons_path))):
                if module not in addons_module:
                    manifest_path = os.path.join(addons_path, module, '__yuancloud__.py')
                    ...
                    if os.path.isfile(manifest_path) and os.path.isdir(path_static):
                        ...
                        if 'yuancloud.addons' in sys.modules:
                            m = __import__('yuancloud.addons.' + module)
                        else:
                            m = None
        ...
        path = os.path.realpath(os.path.join(os.path.dirname(__file__), '..', 'document'))
        statics.update({'/doc': path})
    ...
    def setup_session(self, httprequest):
        ...
        if not sid:
            sid = httprequest.headers.get("X-yuancloud-Session-Id")
        if not sid:
    ...
    def setup_db(self, httprequest):
        ...
        if db:
            dbs = db_filter([db], httprequest=httprequest)
            if db not in dbs:
                _logger.warn("Logged into database '%s', but dbfilter %s "
                             "rejects it; logging session out.", db, dbs)
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    yuancloud.modules.registry.RegistryManager.check_registry_signaling(db)
                    try:
                        with yuancloud.tools.mute_logger('yuancloud.sql_db'):
                        ...
                        if request.httprequest.path == '/web':
                            result = werkzeug.utils.redirect('/web/login')
                        else:
                            result = _dispatch_nodb()
                    else:
                        result = ir_http._dispatch()
                        yuancloud.modules.registry.RegistryManager.signal_caches_change(db)
                else:
                    redirect = '/page/start'
                    h = httprequest.environ.get('HTTP_HOST', '').split(':')[0]
                    if h != 'oddo':
                        d, _, host = h.partition('.')
                        urlparts = urlparse.urlparse(httprequest.url)
                        url = '{scheme}://www.{server}:{port}{path}'.format(scheme=urlparts.scheme, server=host,
                                                                            port=urlparts.port or '80',
                                                                            path=redirect)
                        # werkzeug.exceptions.abort(werkzeug.utils.redirect(url, 303))
                        result = werkzeug.utils.redirect(url, 303)
                    else:
                        result = _dispatch_nodb()
...
def db_list(force=False, httprequest=None):
    dbs = yuancloud.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        return ['yuancloud']
        # d = r.partition('.')[0]
    elif d == 'oddo' or h == '127.0.0.1':
        return [httprequest.session.db]
    r = yuancloud.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = yuancloud.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
class CommonController(Controller):
    ...
    def jsonrpc(self, service, method, args):
        """ Method used by client APIs to contact YuanCloud. """
