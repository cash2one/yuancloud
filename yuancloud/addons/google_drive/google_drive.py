...
from yuancloud.addons.google_account import TIMEOUT
...
class config(osv.Model):
    ...
    def get_access_token(self, cr, uid, scope=None, context=None):
        ...
        if not google_drive_refresh_token:
            if self.pool['res.users']._is_admin(cr, uid, [uid]):
                ...
                raise yuancloud.exceptions.RedirectWarning(msg, action_id, _('Go to the configuration panel'))
            else:
    ...
    def copy_doc(self, cr, uid, res_id, template_id, name_gdocs, res_model, context=None):
        ...
        if content.get('alternateLink'):
            ...
            request_url = "https://www.googleapis.com/drive/v2/files/%s/permissions?emailMessage=This+is+a+drive+file+created+by+YuanCloud&sendNotificationEmails=false&access_token=%s" % (key, access_token)
