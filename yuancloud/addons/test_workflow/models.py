...
class m(yuancloud.osv.orm.Model):
    ...
    def trigger(self, cr, uid, context=None):
        return yuancloud.workflow.trg_trigger(uid, 'test.workflow.trigger', 1, cr)

class n(yuancloud.osv.orm.Model):
    """ A model used for the trigger feature. """
    _name = 'test.workflow.trigger'
    _columns = { 'value': yuancloud.osv.fields.boolean('Value') }
    _defaults = { 'value': False }

class a(yuancloud.osv.orm.Model):
    _name = 'test.workflow.model.a'
    _columns = { 'value': yuancloud.osv.fields.integer('Value') }
    _defaults = { 'value': 0 }

class b(yuancloud.osv.orm.Model):
    _name = 'test.workflow.model.b'
    _inherit = 'test.workflow.model.a'

class c(yuancloud.osv.orm.Model):
    _name = 'test.workflow.model.c'
    _inherit = 'test.workflow.model.a'

class d(yuancloud.osv.orm.Model):
    _name = 'test.workflow.model.d'
    _inherit = 'test.workflow.model.a'

class e(yuancloud.osv.orm.Model):
...
for name in 'bcdefghijkl':
    ...
    #
    class NewModel(yuancloud.osv.orm.Model):
