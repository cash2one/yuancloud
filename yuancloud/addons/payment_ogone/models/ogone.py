...
from yuancloud.addons.payment_ogone.controllers.main import OgoneController
from yuancloud.addons.payment_ogone.data import ogone
from yuancloud.osv import osv, fields
from yuancloud.tools import float_round, DEFAULT_SERVER_DATE_FORMAT
from yuancloud.tools.float_utils import float_compare, float_repr
...
class PaymentAcquirerOgone(osv.Model):
    ...
    def _ogone_generate_shasign(self, acquirer, inout, values):
        ...
        :param string inout: 'in' (yuancloud contacting ogone) or 'out' (ogone
                             contacting yuancloud). In this last case only some
    ...
    def ogone_form_generate_values(self, cr, uid, id, values, context=None):
        ...
        if values.get('type') == 'form_save':
            temp_ogone_tx_values.update({
                'ALIAS': 'YuanCloud-NEW-ALIAS-%s' % time.time(),    # something unique,
...
class PaymentTxOgone(osv.Model):
    ...
    def ogone_s2s_do_transaction(self, cr, uid, id, context=None, **kwargs):
        ...
        reference = tx.reference or "YuanCloud-%s-%s" % (datetime.datetime.now().strftime('%y%m%d_%H%M%S'), tx.partner_id.id)
...
class PaymentMethod(osv.Model):
    ...
    def ogone_create(self, cr, uid, values, context=None):
        if values.get('cc_number'):
            ...
            alias = 'YuanCloud-NEW-ALIAS-%s' % time.time()
