            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: yuancloud.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = yuancloud.__DEBUG__.get_dependencies( job.name );
                ...
                var missing = yuancloud.__DEBUG__.get_missing_jobs();
                var failed = yuancloud.__DEBUG__.get_failed_jobs();
                ...
                if (yuancloud.debug && !_.isEmpty(debug_jobs)) log.push('\nDebug:                  ', debug_jobs);

                if (yuancloud.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                ...
                try {
                    ...
                    $.when(job.factory.call(null, require)).then(
                        function (data) {
                            ...
                            clearTimeout(time);
                            time = _.defer(yuancloud.process_jobs, jobs, services);
                            def.resolve();
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
