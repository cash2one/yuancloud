yuancloud.define('web.data', function (require) {
"use strict";
 ...
 * consumed by YuanCloud's RPC APIs.
 *
...
var Query = Class.extend({
     ...
     * @returns {jQuery.Deferred<Array<yuancloud.web.QueryGroup>> | null}
     */
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of YuanCloud records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the YuanCloud model this dataset will manage
     */
    ...
    get_id_index: function(id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // YuanCloud ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // YuanCloud ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, YuanCloud domain
     ...
     * an SQL-like form usable by YuanCloud's ORM.
     *
     ...
     * YuanCloud's ORM (such as 'name desc', 'product_id', 'order_date asc')
     */
