...
var Session = core.Class.extend(mixins.EventDispatcherMixin, {
    ...
    init: function(parent, origin, options) {
        ...
        options = options || {};
        this.module_list = (options.modules && options.modules.slice()) || (window.yuancloud._modules && window.yuancloud._modules.slice()) || [];
        this.server = null;
    ...
    user_has_group: function(group) {
        ...
        if (!def) {
            var Model = window.yuancloud.web.Model;
    ...
    load_currencies: function() {
        this.currencies = {};
        var self = this;
        return new yuancloud.web.Model("res.currency").query(["symbol", "position", "decimal_places"]).all()
                .then(function(value) {
    ...
    on_modules_loaded: function() {
        var yuancloud = window.yuancloud;
        for(var j=0; j<this.module_list.length; j++) {
            ...
            yuancloud[mod] = {};
            // init module mod
            var fct = yuancloud._yuancloud[mod];
            if(typeof(fct) === "function") {
                yuancloud._yuancloud[mod] = {};
                for (var k in fct) {
                    yuancloud._yuancloud[mod][k] = fct[k];
                }
                fct(yuancloud, yuancloud._yuancloud[mod]);
            }
        ...
        return self.check_session_id().then(function() {
            ...
            if (self.origin_server) {
                ...
                if (self.override_session) {
                    options.headers["X-yuancloud-Session-Id"] = self.session_id || '';
                }
...
yuancloud.define('web.config', function (require) {
"use strict";
