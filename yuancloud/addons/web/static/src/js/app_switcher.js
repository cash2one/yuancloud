yuancloud.define('web.AppSwitcher', function (require) {
"use strict";
...
var AppSwitcher = Widget.extend({
    ...
    willStart: function() {
        ...
        bg.src = '/web/static/src/img/tensor_bg.png';
    ...
    enterprise_expiration_check: function() {
        ...
        if (!yuancloud.db_info) {
            ...
            $.when(
                ...
                // don't show the expiration warning for portal users
                if (!(yuancloud.db_info.warning && yuancloud.db_info.expiration_date))  {
                    return;
                }
                var today = new moment();
                var dbexpiration_date = new moment(yuancloud.db_info.expiration_date);
                var duration = moment.duration(dbexpiration_date.diff(today));
                var options = {
                    'diffDays': Math.round(duration.asDays()),
                    'dbexpiration_reason': yuancloud.db_info.expiration_reason,
                    'warning': yuancloud.db_info.warning,
                    'dbenterprise_code': dbenterprise_code
                };
                self.enterprise_show_panel(options);
            });
    ...
    enterprise_buy: function(ev) {
        ...
        new Model("res.users").call("search_count", [[["share", "=", false],["login_date", ">=", limit_date]]]).then(function(users) {
            window.location = "https://www.yuancloud.cn/yuancloud-enterprise/upgrade?num_users=" + users;
        });
    },
    enterprise_renew: function(ev) {
        new Model('ir.config_parameter').call('get_param', ['database.enterprise_code']).then(function(code) {
            window.location = "https://www.yuancloud.cn/yuancloud-enterprise/renew?code=" + code;
        });
    },
    enterprise_upsell: function(ev) {
        ...
        new Model('ir.config_parameter').call('get_param', ['database.enterprise_code']).then(function(code) {
            new Model("res.users").call("search_count", [[["share", "=", false],["login_date", ">=", limit_date]]]).then(function(users) {
                window.location = "https://www.yuancloud.cn/yuancloud-enterprise/upsell?num_users=" + users + "&code=" + code;
            });
        });
    },
