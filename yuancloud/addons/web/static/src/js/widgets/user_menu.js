yuancloud.define('web.UserMenu', function (require) {
"use strict";
...
var UserMenu = Widget.extend({
    ...
    on_menu_documentation: function () {
        window.open('https://www.yuancloud.cn/documentation/user', '_blank');
    },
    on_menu_support: function () {
        window.location.href = 'mailto:help@yuancloud.cn';
    },
    on_menu_about: function () {
        var self = this;
        if (yuancloud.db_info) {
            menu_help_about(yuancloud.db_info);
        } else {
    ...
    on_menu_account: function() {
        ...
        P.call('get_param', ['database.uuid']).then(function(dbuuid) {
            ...
            window.location.href = 'https://accounts.yuancloud.cn/oauth2/auth?'+$.param(params);
        }).fail(function(result, ev){
            ev.preventDefault();
            window.location.href = 'https://accounts.yuancloud.cn/account';
        });
    },
