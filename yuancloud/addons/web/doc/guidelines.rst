...
example in YuanCloud is dashboards), queries should be restricted to a
...
YuanCloud Web's :js:class:`~yuancloud.web.Widget` has an attribute
providing its DOM root :js:attr:`Widget.$el <yuancloud.web.Widget.$el>`,
and a shortcut to select nodes directly :js:attr:`Widget.$
<yuancloud.web.Widget.$>`.
...
* All interactive components (components displaying information to the
  ...
  :class:`~yuancloud.web.Widget` and correctly implement and use its API
  and lifecycle.
