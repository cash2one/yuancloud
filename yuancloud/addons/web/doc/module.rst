...
Building an YuanCloud Web module
==============================

There is no significant distinction between an YuanCloud Web module and
an YuanCloud module, the web part is mostly additional data and code
inside a regular YuanCloud module. This allows providing more seamless
features by integrating your module deeper into the web client.

A Basic Module
--------------

A very basic YuanCloud module structure will be our starting point:
...
This is a sufficient minimal declaration of a valid YuanCloud module.

Web Declaration
---------------

There is no such thing as a "web module" declaration. An YuanCloud
...
.. note::
    ...
    restart the YuanCloud server itself for it to be taken in account.
...
provides: an instance of the YuanCloud Web client, with the contents of
...
its :js:func:`~yuancloud.web.Widget.start` to add some content to its
DOM:
...
will require restarting the YuanCloud Server to see the changes, as
usual):
...
.. note::
    ...
    Prefixing CSS rules with both ``.yuancloud`` (to ensure the rule
    will apply only within the confines of the YuanCloud Web client) and
...
be done in :js:func:`~yuancloud.web.Widget.start` but that gets unwieldy
...
YuanCloud Web's template language is :doc:`qweb`. Although any
...
integration to YuanCloud Web widgets.
...
To prevent this irremediable loss, we can use YuanCloud's support for
...
:js:func:`~yuancloud.base.Widget.start()` is to perform *asynchronous*
...
recorded previously using the :js:class:`~yuancloud.web.Query` interface
...
4. And finally — after restarting YuanCloud — navigating to the test
...
side-effect of this is to :js:func:`~yuancloud.web.Widget.start` it,
...
:js:func:`set a widget's DOM <yuancloud.web.Widget.setElement>`, let's
