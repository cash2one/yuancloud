...
import yuancloud
import yuancloud.modules.registry
from yuancloud.addons.base.ir.ir_qweb import AssetsBundle, QWebTemplateNotFound
from yuancloud.modules import get_module_path, get_resource_path
from yuancloud.tools import topological_sort
from yuancloud.tools.translate import _
from yuancloud.tools import ustr
from yuancloud import http
import mimetypes
from yuancloud.http import request, serialize_exception as _serialize_exception, STATIC_CACHE
from yuancloud.exceptions import AccessError
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('yuancloud.addons.web', "views")
...
def ensure_db(redirect='/web/login'):
    ...
    r = request.httprequest
    h = r.environ.get('HTTP_HOST', '').split(':')[0]
    if db and db != 'yuancloud' and db != h:
        urlparts = urlparse.urlparse(r.url)
        # urlparts_list = list(urlparts)
        # urlparts_list[1] = "{host}"
        url = '{scheme}://{server}:{port}{path}'.format(scheme=urlparts.scheme, server=db, port=urlparts.port or '80',
                                                        path=urlparts.path)

        if not request.session.db:
            request.session.db = db
        abort_and_redirect(url)
        return
    ...
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, yuancloud.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}
...
def module_boot(db=None):
    server_wide_modules = yuancloud.conf.server_wide_modules or ['web']
    serverside = []
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("yuancloud.addons.web.main.manifest_list(): debug parameter is deprecated")
...
def fix_view_modes(action):
    """ For historical reasons, YuanCloud has weird dealings in relation to
...
def _local_web_translations(trans_file):
    ...
    try:
        ...
        return
    for x in po:
        if x.id and x.string and "yuancloud-web" in x.auto_comments:
...
def db_info():
    version_info = yuancloud.service.common.exp_version()
    return {
...
class Home(http.Controller):
    ...
    def web_client(self, s_action=None, **kw):
        ...
        menu_data = request.registry['ir.ui.menu'].load_menus(request.cr, request.uid, request.debug,
                                                              context=request.context)
        return request.render('web.webclient_bootstrap',
                              qcontext={'menu_data': menu_data, 'db_info': json.dumps(db_info())})
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = yuancloud.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def load_locale(self, lang):
        ...
        for code in magic_file_finding:
            try:
                with open(os.path.join(addons_path, 'web', 'static', 'lib', 'moment', 'locale', code + '.js'),
                          'r') as f:
    ...
    def version_info(self):
        return yuancloud.service.common.exp_version()
...
class Proxy(http.Controller):
    ...
    def post(self, path):
        ...
        with request.session.load_request_data() as data:
            ...
            headers = {'X-yuancloud-Session-Id': request.session.sid}
...
class Database(http.Controller):
    def _render_template(self, **d):
        ...
        d['insecure'] = yuancloud.tools.config['admin_passwd'] == 'admin'
        d['list_db'] = yuancloud.tools.config['list_db']
        d['langs'] = yuancloud.service.db.exp_list_lang()
        # databases list
        d['databases'] = []
        try:
            if 'force' in d:
                d['databases'] = yuancloud.service.db.list_dbs(True)
            else:
    ...
    def selector(self, **kw):
        return self._render_template(manage=False, **kw)
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        try:
            yuancloud.service.db.check_super(master_pwd)
            ...
            dump_stream = yuancloud.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=yuancloud.SUPERUSER_ID))
...
class Binary(http.Controller):
    ...
    @http.route(['/web/image',
                ...
                auth="public")
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas', filename_field='datas_fname',
        ...
        if content and width and height:
            ...
            if height > 500:
                height = 500
            content = yuancloud.tools.image_resize_image(base64_source=content, size=(width, height), encoding='base64',
                                                         filetype='PNG')
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        uid = None
        if dbname is None and request.session.db:
        ...
        if not dbname:
            ...
            try:
                ...
                registry = yuancloud.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name='',
        ...
        fields_sequence = sorted(fields.iteritems(),
                                 key=lambda field: yuancloud.tools.ustr(field[1].get('string', '')))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from YuanCloud's export data to whatever the
