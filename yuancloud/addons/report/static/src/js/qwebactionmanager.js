yuancloud.define('report.report', function (require) {
        ...
        if ('report_type' in action && (action.report_type == 'qweb-html' || action.report_type == 'qweb-pdf' || action.report_type == 'controller')) {
            ...
            if (action.report_type == 'qweb-html') {
                ...
                (wkhtmltopdf_state = wkhtmltopdf_state || session.rpc('/report/check_wkhtmltopdf')).then(function (presence) {
                    // Fallback on html if wkhtmltopdf is not installed or if YuanCloud is started with one worker
                    if (presence === 'install') {
...
wkhtmltopdf.org</a>'), true);
                        ...
                        self.do_notify(_t('Report'), _t('You need to start YuanCloud with at least two \
