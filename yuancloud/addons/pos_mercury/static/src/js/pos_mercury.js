yuancloud.define('pos_mercury.pos_mercury', function (require) {
"use strict";
...
PaymentScreenWidget.include({
    // How long we wait for the yuancloud server to deliver the response of
    ...
    credit_code_transaction: function (parsed_result, old_deferred, retry_nr) {
        ...
        mercury_transaction.call('do_payment', [transaction], undefined, {timeout: self.server_timeout_in_ms}).then(function (data) {
            ...
            if (data === "internal error") {
                def.resolve({
                    message: _t("YuanCloud error while processing transaction.")
                });
                return;
            }
