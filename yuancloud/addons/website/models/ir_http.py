...
import yuancloud
from yuancloud.addons.base import ir
from yuancloud.addons.base.ir import ir_qweb
from yuancloud.addons.website.models.website import slug, url_for, _UNSLUG_RE
from yuancloud.http import request
from yuancloud.tools import config
...
class ir_http(orm.AbstractModel):
    ...
    def _auth_method_public(self):
        if not request.session.uid:
            website = self.pool['website'].get_current_website(request.cr, yuancloud.SUPERUSER_ID, context=request.context)
            if website:
                request.uid = website.user_id.id
            else:
                request.uid = self.pool['ir.model.data'].xmlid_to_res_id(request.cr, yuancloud.SUPERUSER_ID, 'base', 'public_user')
        else:
        ...
        if cache_enable:
            ...
            try:
                ...
                if r['time'] + cache_time > time.time():
                    cache_response = yuancloud.http.Response(r['content'], mimetype=r['mimetype'])
                else:
    ...
    def _handle_exception(self, exception, code=500):
        ...
        if not is_website_request:
            ...
            try:
                ...
                if yuancloud.tools.config['dev_mode'] and (not isinstance(exception, ir_qweb.QWebException) or not exception.qweb.get('cause')):
                    raise
                exception = e
            ...
            if isinstance(exception, yuancloud.exceptions.AccessError):
                code = 403

            if isinstance(exception, ir_qweb.QWebException):
                values.update(qweb_exception=exception)
                if isinstance(exception.qweb.get('cause'), yuancloud.exceptions.AccessError):
                    code = 403
