...
from yuancloud.osv import orm, osv, fields
from yuancloud.tools import html_escape as escape, ustr, image_resize_and_sharpen, image_save_for_web
from yuancloud.tools.safe_eval import safe_eval
...
def is_multilang_url(local_url, langs=None):
    ...
    try:
        ...
        # Force to check method to POST. YuanCloud uses methods : ['POST'] and ['GET', 'POST']
...
class website(osv.osv):
    ...
    _defaults = {
        'user_id': lambda self,cr,uid,c: self.pool['ir.model.data'].xmlid_to_res_id(cr, yuancloud.SUPERUSER_ID, 'base.public_user'),
        'company_id': lambda self,cr,uid,c: self.pool['ir.model.data'].xmlid_to_res_id(cr, yuancloud.SUPERUSER_ID,'base.main_company'),
        'compress_html': False,
        'cdn_activated': False,
    ...
    def new_page(self, cr, uid, name, template='website.default_page', ispage=True, context=None):
        ...
        while view.search(cr, yuancloud.SUPERUSER_ID, [('key', '=', page_xmlid), '|'] + dom, context=dict(context or {}, active_test=False)):
            inc += 1
    ...
    @yuancloud.tools.ormcache('domain_name')
...
class res_partner(osv.osv):
    ...
    def google_map_img(self, cr, uid, ids, zoom=8, width=298, height=298, context=None):
        ...
        params = {
            ...
            'center': '%s %s' % (partner.city or '',partner.street or '' ),
			'size': "%sx%s" % (height, width),
            'zoom': zoom,
            'sensor': 'false',
        }
        return urlplus('//ditu.google.cn/maps/api/staticmap' , params)
    ...
    def google_map_link(self, cr, uid, ids, zoom=10, context=None):
        ...
        return urlplus('https://maps.google.cn/maps' , params)

class res_company(osv.osv):
    _inherit = "res.company"
    def google_map_img(self, cr, uid, ids, zoom=8, width=298, height=298, context=None):
        partner = self.browse(cr, yuancloud.SUPERUSER_ID, ids[0], context=context).partner_id
