...
Also, adds methods to convert values back to yuancloud models.
"""
...
import yuancloud
from yuancloud.osv import orm, fields
from yuancloud.tools import ustr, DEFAULT_SERVER_DATE_FORMAT, DEFAULT_SERVER_DATETIME_FORMAT
from yuancloud.tools import html_escape as escape
...
class DateTime(orm.AbstractModel):
    ...
    def from_html(self, cr, uid, model, field, element, context=None):
        ...
        tz_name = context.get('tz') \
            or self.pool['res.users'].read(cr, yuancloud.SUPERUSER_ID, uid, ['tz'], context=context)['tz']
        if tz_name:
            try:
...
class Image(orm.AbstractModel):
    ...
    def load_local_url(self, url):
        ...
        path = yuancloud.modules.get_module_resource(
