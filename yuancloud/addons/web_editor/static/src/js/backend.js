yuancloud.define('web_editor.backend', function (require) {
'use strict';
 ...
 * widget will instantiate an iframe with the editor summernote improved by yuancloud
 */
...
var FieldTextHtml = widget.extend({
    ...
    start: function () {
        ...
        window.yuancloud[this.callback+"_editor"] = function (EditorBar) {
            setTimeout(function () {
                self.on_editor_loaded(EditorBar);
            },0);
        };
        window.yuancloud[this.callback+"_content"] = function (EditorBar) {
            self.on_content_loaded();
        };
        window.yuancloud[this.callback+"_updown"] = null;
        window.yuancloud[this.callback+"_downup"] = function (value) {
            self.internal_set_value(value);
            self.trigger('changed_value');
            self.resize();
        };
    ...
    on_editor_loaded: function (EditorBar) {
        ...
        if (this.get('value') && window.yuancloud[self.callback+"_updown"] && !(this.$content.html()||"").length) {
            this.render_value();
        }
        setTimeout(function () {
            setTimeout(self.resize,0);
    ...
    render_value: function() {
        ...
        if (!this.get("effective_readonly")) {
            if(window.yuancloud[this.callback+"_updown"]) {
                window.yuancloud[this.callback+"_updown"](value, this.view.get_fields_values(), this.name);
                this.resize();
            }
        } else {
    ...
    destroy: function () {
        ...
        delete window.yuancloud[this.callback+"_editor"];
        delete window.yuancloud[this.callback+"_content"];
        delete window.yuancloud[this.callback+"_updown"];
        delete window.yuancloud[this.callback+"_downup"];
    }
});
