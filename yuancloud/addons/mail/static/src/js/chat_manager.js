yuancloud.define('mail.chat_manager', function (require) {
"use strict";
...
function send_native_notification(title, content) {
    var notification = new Notification(title, {body: content, icon: "/mail/static/src/img/yuancloud_o.png"});
...
function notify_incoming_message (msg, options) {
    if (bus.is_yuancloud_focused() && options.is_displayed) {
        // no need to notify
        return;
    }
