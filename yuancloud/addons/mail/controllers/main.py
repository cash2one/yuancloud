...
from yuancloud.addons.web.controllers.main import binary_content
        ...
        if res_model in request.env:
            try:
                ...
                if partner_id in request.env[res_model].browse(res_id).sudo().exists().message_ids.mapped('author_id').ids:
                    status, headers, content = binary_content(model='res.partner', id=partner_id, field='image_medium', default_mimetype='image/png', env=request.env(user=yuancloud.SUPERUSER_ID))
                    if status == 304:
