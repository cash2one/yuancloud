...
from yuancloud import report as yuancloud_report
...
def format_tz(pool, cr, uid, dt, tz=False, format=False, context=None):
    ...
    ts = yuancloud.osv.fields.datetime.context_timestamp(cr, uid, timestamp, context)
            ...
            if template.report_template and not 'report_template_in_attachment' in self.env.context:
                for res_id in template_res_ids:
                    ...
                    if report.report_type in ['qweb-html', 'qweb-pdf']:
                        ...
                        result, format = yuancloud_report.render_report(self._cr, self._uid, [res_id], report_service, {'model': template.model}, Template._context)
