yuancloud.define('web_settings_dashboard', function (require) {
"use strict";
...
var DashboardApps = Widget.extend({
    ...
    start: function() {
        this._super.apply(this, arguments);
        if (yuancloud.db_info && yuancloud.db_info.server_version_info[5] === 'c') {
    ...
    confirm_upgrade: function() {
        framework.redirect("http://www.yuancloud.cn/yuancloud-enterprise/upgrade?num_users=" + (this.data.enterprise_users || 1));
    },
});
...
var DashboardShare = Widget.extend({
    ...
    init: function(parent, data){
        this.data = data;
        this.parent = parent;
        this.share_url = 'http://www.yuancloud.cn';
        this.share_text = encodeURIComponent("I am using #YuanCloud - Awesome open source business apps.");
    },
    ...
    share_linkedin: function(){
        var popup_url = _.str.sprintf('http://www.linkedin.com/shareArticle?mini=true&url=%s&title=I am using yuancloud&summary=%s&source=www.yuancloud.cn', encodeURIComponent(this.share_url), this.share_text);
        this.sharer(popup_url);
    },
