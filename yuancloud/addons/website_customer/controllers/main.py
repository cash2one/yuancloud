...
from yuancloud.addons.website.models.website import unslug
...
class WebsiteCustomer(http.Controller):
    ...
    def customers(self, country_id=0, page=0, country_name='', tag_id=0, **post):
        ...
        countries = partner_obj.read_group(
            cr, yuancloud.SUPERUSER_ID, domain, ["id", "country_id"],
            groupby="country_id", orderby="country_id", context=request.context)
        country_count = partner_obj.search(
            cr, yuancloud.SUPERUSER_ID, domain, count=True, context=request.context)
        ...
        partner_count = partner_obj.search_count(cr, yuancloud.SUPERUSER_ID, domain, context=request.context)
        ...
        partner_ids = partner_obj.search(request.cr, yuancloud.SUPERUSER_ID, domain,
                                         offset=pager['offset'], limit=self._references_per_page,
                                         context=request.context)
        google_map_partner_ids = ','.join(map(str, partner_ids))
        partners = partner_obj.browse(request.cr, yuancloud.SUPERUSER_ID, partner_ids, request.context)
