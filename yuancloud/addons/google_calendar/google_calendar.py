...
from yuancloud.tools import DEFAULT_SERVER_DATE_FORMAT, DEFAULT_SERVER_DATETIME_FORMAT, exception_to_unicode
...
class SyncEvent(object):
    ...
    def compute_OP(self, modeFull=True):
        #If event are already in Gmail and in YuanCloud
        ...
        # New in yuancloud...  Create on create_events of synchronize function
        elif self.OE.found and not self.GG.found:
            if self.OE.status:
                self.OP = Delete('OE', 'Update or delete from GOOGLE')
            else:
                if not modeFull:
                    self.OP = Delete('GG', 'Deleted from YuanCloud, need to delete it from Gmail if already created')
                else:
                    self.OP = NothingToDo("", "Already Deleted in gmail and unlinked in YuanCloud")
...
class google_calendar(osv.AbstractModel):
    ...
    def update_events(self, cr, uid, lastSync=False, context=None):
        ...
        if lastSync:
            ...
            my_yuancloud_att_ids = att_obj.search(cr, uid, [
                ('partner_id', '=', myPartnerID),
            ...
            my_yuancloud_googleinternal_ids = att_obj.read(cr, uid, my_yuancloud_att_ids, ['google_internal_event_id', 'event_id'], context=context_novirtual)

            if self.get_print_log(cr, uid, context=context):
                _logger.info("Calendar Synchro -  \n\nUPDATE IN GOOGLE\n%s\n\nRETRIEVE FROM OE\n%s\n\nUPDATE IN OE\n%s\n\nRETRIEVE FROM GG\n%s\n\n" % (all_event_from_google, my_google_att_ids, my_yuancloud_att_ids, my_yuancloud_googleinternal_ids))

            for giid in my_yuancloud_googleinternal_ids:
            ...
            my_att_ids = list(set(my_google_att_ids + my_yuancloud_att_ids))
            ...
            # Select all events from YuanCloud which have been already synchronized in gmail
...
#################################
    ...
    def authorize_google_uri(self, cr, uid, from_url='http://www.yuancloud.cn', context=None):
...
all links between GoogleID and YuanCloud Google Internal ID')
    }
...
class calendar_event(osv.Model):
    ...
    _columns = {
        'oe_update_date': fields.datetime('YuanCloud Update Date'),
    }
...
class calendar_attendee(osv.Model):
    ...
    _columns = {
        ...
        'oe_synchro_date': fields.datetime('YuanCloud Synchro Date'),
    }
