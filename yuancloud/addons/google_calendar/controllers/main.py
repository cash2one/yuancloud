import yuancloud.addons.web.http as http
...
class google_calendar_controller(http.Controller):
    ...
    def sync_data(self, arch, fields, model, **kw):
        """
            This route/function is called when we want to synchronize yuancloud calendar with Google Calendar
            Function return a dictionary with the status :  need_config_from_admin, need_auth, need_refresh, success if not calendar_event
            The dictionary may contains an url, to allow YuanCloud Client to redirect user on this URL for authorization for example
        """
        ...
        if model == 'calendar.event':
            ...
            # Checking that user have already accepted YuanCloud to access his calendar !
    ...
    def remove_references(self, model, **kw):
        """
            This route/function is called when we want to remove all the references between one calendar YuanCloud and one Google Calendar
        """
        status = "NOP"
        if model == 'calendar.event':
            gc_obj = request.registry['google.calendar']
