...
import yuancloud
from yuancloud.exceptions import UserError, ValidationError, QWebException
from yuancloud.tools.translate import translate
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
                ...
                try:
                    if args and len(args) > 1:
                        ...
                        if len(args) > 3 and isinstance(args[3], (long, int, list)):
                            ...
                            ids = []
                    cr = yuancloud.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if yuancloud.registry(dbname)._init and not yuancloud.tools.config['test_enable']:
                    raise yuancloud.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
                ...
                registry = yuancloud.registry(dbname)
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    object = yuancloud.registry(cr.dbname).get(obj)
    if object is None:
