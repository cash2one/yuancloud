...
# User code must use the exceptions defined in ``yuancloud.exceptions`` (not
...
def xmlrpc_return(start_response, service, method, params, string_faultcode=False):
    ...
    """
    # Map YuanCloud core exceptions to XML-RPC fault codes. Specific exceptions
    # defined in ``yuancloud.exceptions`` are mapped to specific fault codes;
    ...
    # exception handling.
    try:
        result = yuancloud.http.dispatch_rpc(service, method, params)
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, yuancloud.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, yuancloud.tools.ustr(e.value))
        ...
        response = xmlrpclib.dumps(fault, allow_none=False, encoding=None)
    elif isinstance(e, yuancloud.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = yuancloud.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, yuancloud.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(yuancloud.tools.ustr(e.message), formatted_info)
        response = xmlrpclib.dumps(fault, allow_none=False, encoding=None)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(yuancloud.tools.exception_to_unicode(e), formatted_info)
        response = xmlrpclib.dumps(fault, allow_none=None, encoding=None)
    return response
...
def application_unproxied(environ, start_response):
    ...
    # web.session.yuancloudSession.send() and at RPC dispatch in
    # yuancloud.service.web_services.objects_proxy.dispatch().
    ...
    with yuancloud.api.Environment.manage():
        ...
        for handler in [wsgi_xmlrpc, yuancloud.http.root]:
