...
from yuancloud.release import nt_service_name
import yuancloud.tools.config as config
from yuancloud.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'yuancloud.service.http.request.%s' % (me.ident,)
...
class FSWatcher(object):
    def __init__(self):
        ...
        for path in yuancloud.modules.module.ad_paths:
...
class ThreadedServer(CommonServer):
    ...
    def signal_handler(self, sig, frame):
        if sig in [signal.SIGINT, signal.SIGTERM]:
            ...
            # restart on kill -HUP
            yuancloud.phoenix = True
            self.quit_signals_received += 1

    def cron_thread(self, number):
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = yuancloud.modules.registry.RegistryManager.registries
            _logger.debug('cron%d polling for jobs', number)
            for db_name, registry in registries.iteritems():
                while registry.ready:
                    acquired = yuancloud.addons.base.ir.ir_cron.ir_cron._acquire_job(db_name)
                    if not acquired:
                        break
    ...
    def cron_spawn(self):
        ...
        for i in range(yuancloud.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="yuancloud.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="yuancloud.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        _logger.debug('--')
        yuancloud.modules.registry.RegistryManager.delete_all()
        logging.shutdown()
...
class PreforkServer(CommonServer):
    ...
    def long_polling_spawn(self):
        ...
        cmd = nargs[0]
        cmd = os.path.join(os.path.dirname(cmd), "yuancloud-gevent")
        nargs[0] = cmd
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('yuancloud: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(yuancloud.modules.registry.RegistryManager.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = yuancloud.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('yuancloud.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            import yuancloud.addons.base as base
...
def load_server_wide_modules():
    for m in yuancloud.conf.server_wide_modules:
        try:
            yuancloud.modules.module.load_yuancloud_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `yuancloud-web` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute yuancloud-server process with (nearly) the same arguments"""
    if yuancloud.tools.osutil.is_running_as_nt_service():
...
def load_test_file_yml(registry, test_file):
    with registry.cursor() as cr:
        yuancloud.tools.convert_yaml_import(cr, 'base', file(test_file), 'test', {}, 'init')
    ...
    for mod_name, mod_mod in sys.modules.items():
        if mod_mod:
            ...
            if test_path == mod_path:
                ...
                stream = yuancloud.modules.module.TestStream()
...
def start(preload=None, stop=False):
    """ Start the yuancloud http server and cron processor.
    """
    global server
    load_server_wide_modules()
    if yuancloud.evented:
        server = GeventServer(yuancloud.service.wsgi_server.application)
    elif config['workers']:
        server = PreforkServer(yuancloud.service.wsgi_server.application)
    else:
        server = ThreadedServer(yuancloud.service.wsgi_server.application)
    ...
    if getattr(yuancloud, 'phoenix', False):
        if watcher:
            watcher.stop()
        _reexec()
