...
from yuancloud.tools import frozendict, classproperty
...
def constrains(*args):
    ...
    Should raise :class:`~yuancloud.exceptions.ValidationError` if the
    validation failed.
...
def make_wrapper(decorator, method, old_api, new_api):
    ...
    # propagate specific yuancloud attributes from method to wrapper
...
def one(method):
    """ Decorate a record-style method where ``self`` is expected to be a
        ...
        .. deprecated:: 9.0
            ...
            is a single record with :meth:`~yuancloud.models.Model.ensure_one`.
    """
    split = get_context_split(method)
    downgrade = get_downgrade(method)
...
from yuancloud.exceptions import UserError, AccessError, MissingError
