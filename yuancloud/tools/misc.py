...
"""
Miscellaneous tools used by YuanCloud.
"""
...
from yuancloud.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if yuancloud.tools.config['db_host']:
        env['PGHOST'] = yuancloud.tools.config['db_host']
    if yuancloud.tools.config['db_port']:
        env['PGPORT'] = str(yuancloud.tools.config['db_port'])
    if yuancloud.tools.config['db_user']:
        env['PGUSER'] = yuancloud.tools.config['db_user']
    if yuancloud.tools.config['db_password']:
        env['PGPASSWORD'] = yuancloud.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the YuanCloud root, using a subdir folder.
    ...
    """
    import yuancloud.modules as addons
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the YuanCloud root: skip zipfile lookup.
...
def scan_languages():
    """ Returns all languages supported by YuanCloud for translation
...
class upload_data_thread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            fp = urllib.urlopen('http://www.yuancloud.cn/scripts/survey.php', args)
            fp.read()
            fp.close()
        except Exception:
            pass
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns 
       ...
       the evaluation.
       Mostly useful for evaluating YuanCloud domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('yuancloud.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('yuancloud.foo.bar'):
            do_suff()
