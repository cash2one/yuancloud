...
def _get_idref(self, cr, uid, model_str, context, idref):
    idref2 = dict(idref,
                  ...
                  version=yuancloud.release.major_version,
                  ref=_ref(self, cr),
                  pytz=pytz)
    if len(model_str):
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('yuancloud.tools.convert.init').error(
        ...
        data = node.text
        if node.get('file'):
            with yuancloud.tools.file_open(node.get('file'), 'rb') as f:
                data = f.read()
...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_workflow(self, cr, rec, data_node=None, mode=None):
        ...
        uid = self.get_uid(cr, self.uid, data_node, rec)
        yuancloud.workflow.trg_validate(
    ...
    def parse(self, de, mode=None):
        roots = ['yuancloud','data','yuancloud']
        if de.tag not in roots:
            raise Exception("Root xml tag must be <yuancloud>, <yuancloud> or <data>.")
        for rec in de:
            if rec.tag in roots:
                self.parse(rec, mode)
