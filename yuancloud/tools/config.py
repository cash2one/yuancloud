#yuancloud.loggers.handlers. -*- coding: utf-8 -*-
...
import sys
import yuancloud
import yuancloud.conf
import yuancloud.loglevels as loglevels
import logging
import yuancloud.release as release
import appdirs
...
def _deduplicate_loggers(loggers):
    ...
    if for some odd reason people use :option`yuancloud.py --save`` all the time.
    """
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.sswy.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        }
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.yuancloud_serverrc")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store YuanCloud data")
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "yuancloud.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="yuancloud.http.rpc.request:DEBUG", help='shortcut for --log-handler=yuancloud.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="yuancloud.http.rpc.response:DEBUG", help='shortcut for --log-handler=yuancloud.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="yuancloud.http:DEBUG", help='shortcut for --log-handler=yuancloud.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="yuancloud.sql_db:DEBUG", help='shortcut for --log-handler=yuancloud.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate YuanCloud to another language."
    ...
    def parse_config(self, args=None):
        ...
        This method initializes yuancloud.tools.config and yuancloud.conf (the
        ...
        Typical usage of this method:
            ...
            yuancloud.tools.config.parse_config(sys.argv[1:])
        """
        self._parse_config(args)
        yuancloud.netsvc.init_logger()
        yuancloud.modules.module.initialize_sys_path()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'yuancloud-server.conf')
        else:
            rcfilepath = os.path.expanduser('~/.yuancloud_serverrc')

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('yuancloud_SERVER') or rcfilepath)
        self.load()
        ...
        self.options['root_path'] = os.path.abspath(os.path.expanduser(os.path.expandvars(os.path.dirname(yuancloud.__file__))))
        ...
        yuancloud.conf.addons_paths = self.options['addons_path'].split(',')
        if opt.server_wide_modules:
            yuancloud.conf.server_wide_modules = map(lambda m: m.strip(), opt.server_wide_modules.split(','))
        else:
            yuancloud.conf.server_wide_modules = ['web','web_kanban']
    ...
    def _is_addons_path(self, path):
        for f in os.listdir(path):
            ...
            if os.path.isdir(modpath):
                ...
                if hasfile('__init__.py') and (hasfile('__yuancloud__.py') or hasfile('__terp__.py')):
                    return True
        return False
