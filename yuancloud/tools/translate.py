...
import locale
import os
import yuancloud.sql_db as sql_db
import re
import logging
import tarfile
import tempfile
...
WEB_TRANSLATION_COMMENT = "yuancloud-web"
...
def trans_generate(lang, modules, cr):
    ...
    path_list = [(path, True) for path in yuancloud.modules.module.ad_paths]
        ...
        for root, dummy, files in osutil.walksymlinks(path):
            ...
            if fnmatch.fnmatch(root, '*/static/src/xml*'):
                for fname in fnmatch.filter(files, '*.xml'):
                    babel_extract_terms(fname, path, root, 'yuancloud.tools.translate:babel_extract_qweb',
                                        extra_comments=[WEB_TRANSLATION_COMMENT])
            if not recursive:
...
def trans_load_data(cr, fileobj, fileformat, lang, lang_name=None, verbose=True, module_name=None, context=None):
    ...
    if context is None:
        context = {}
    db_name = cr.dbname
    registry = yuancloud.registry(db_name)
