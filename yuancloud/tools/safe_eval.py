...
YuanCloud domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    an YuanCloud domain expression from an untrusted source.
    ...
    except yuancloud.exceptions.except_orm:
        raise
    except yuancloud.exceptions.Warning:
        raise
    except yuancloud.exceptions.RedirectWarning:
        raise
    except yuancloud.exceptions.AccessDenied:
        raise
    except yuancloud.exceptions.AccessError:
        raise
    except OperationalError:
        # Do not hide PostgreSQL low-level exceptions, to let the auto-replay
        # of serialized transactions work its magic
        raise
    except yuancloud.exceptions.MissingError:
        raise
    except Exception, e:
        import sys
        exc_info = sys.exc_info()
